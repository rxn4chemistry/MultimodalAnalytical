...
# one can reduce the patience here since the model will converge faster
...
do
    ...
    python3 scripts/cli/training.py \ 
        working_dir=${run_folder} \ 
        job_name=${task}/finetuned_${i} \ 
        cv_split=${i} \ 
    ...
    cat ${run_folder}/${task}/finetuned_${i}/metrics*
done
