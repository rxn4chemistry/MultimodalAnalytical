...
def smooth_augment(spectrum: np.ndarray, sigmas: List[float]) -> List[np.ndarray]:
...
AUGMENT_OPTIONS = {
    "horizontal": horizontal_shift_augment,
    "smooth": smooth_augment,
    "smiles_aug": smiles_augment,
}
...
def augment(dataset: Dataset, augment_config: Optional[DictConfig], num_cpu: int) -> Dataset:
    ...
    if (
        isinstance(augment_config["augmentations"], ListConfig)
        and len(augment_config["augmentations"]) != 0
    ):
        for augment_fields in augment_config["augmentations"]:
            ...
            augmented_datasets.append(
                dataset.map(
                    lambda row: apply_augment(row, augment_column, augment_fns),
                    batched=True,
                    batch_size=1,
                    num_proc=num_cpu,
                )
...
def apply_augment(row, augment_column: str, augment_config: Dict[str, Any]):
    ...
    for augment_type, augment_params in augment_config.items():
        augmented_data.extend(
            AUGMENT_OPTIONS[augment_type](row[augment_column][0], **augment_params) # type:ignore
        )  # type:ignore

    # Duplicate Data in other columns
    augmented_row = {
        column: row[column] * len(augmented_data)
        for column in row.keys()
        if column != augment_column
    }
