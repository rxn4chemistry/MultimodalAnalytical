...
class GuidedFormulaProcessor(LogitsProcessor):
    ...
    def __init__(self, n_beams: int, chemical_formula: List[str], target_tokenizer: AutoTokenizer):
        """
        Args:
            n_beams: Beam size
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # Decode Smiles
        decoded_smiles = self.target_tokenizer.batch_decode(input_ids, skip_special_tokens=True)
        decoded_smiles = [smiles.replace(" ", "") for smiles in decoded_smiles]
        decoded_smiles = [
            (Chem.MolToSmiles(Chem.MolFromSmiles(smiles)) if Chem.MolFromSmiles(smiles) else "")
            for smiles in decoded_smiles
        ]

        # Decode Formula and make numerical representation
        decoded_formula_list = list()
        for smiles in decoded_smiles:
            try:
        ...
        decoded_formula_matching = np.all(self.chemical_formula_beams == decoded_formula, axis=1)
        scores[decoded_formula_matching, self.eos_token_id] = 0

        # If pred Formula smaller than valid formula, tank eos s
        decoded_formula_too_small = np.any(decoded_formula < self.chemical_formula_beams, axis=1)
        ...
        target_formula = self.chemical_formula_beams.repeat(  # type: ignore
            ...
            decoded_formula.shape[0],
            self.vocab_size,
            len(self.atom_list),  # type: ignore
        )
        ...
        next_formula_too_large = np.any(next_formula[:, :, :9] > target_formula[:, :, :9], axis=2)
