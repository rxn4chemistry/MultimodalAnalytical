...
def reject_sample(predictions: Dict[str, Any], molecules: bool = True):
    ...
    assert len(predictions["predictions"]) == len(predictions["targets"]), (
        f"Predictions and targets do not match in size: {len(predictions['predictions'])} != {len(predictions['targets'])}"
    )

    for i in range(len(predictions["predictions"])):
        assert len(predictions["predictions"][i]) == n_beams, (
            f"{len(predictions['predictions'][i])}/{n_beams}"
        )
...
def calc_sampling_metrics(
    ...
    RDLogger.DisableLog("rdApp.*")
    prediction_df["predictions_clean"] = prediction_df["predictions"].map(
        lambda prediction: [clean_sample(pred, molecules) for pred in prediction]
    )
    prediction_df["targets_clean"] = prediction_df["targets"].map(
        lambda target: clean_sample(target, molecules)
    )

    # Calculate rank
    prediction_df["rank"] = prediction_df.apply(
        lambda row: (
            row["predictions_clean"].index(row["targets_clean"])
            if row["targets_clean"] in row["predictions_clean"]
            else n_beams
        ),
        axis=1,
    )
    ...
    for i in range(n_beams):
        if classes:
            for cl in prediction_df["prediction_classes"].unique():
                ...
                if logging:
                    logger.info(
                        f"Class: {cl}. Samples per class: {len(cls_df)}. Top-{i + 1}: {top_n_acc:.3f}"
                    )
        else:
...
def calculate_training_steps(train_set: Dataset, config: DictConfig) -> int:
    ...
    train_steps = (
        math.ceil(batches_per_gpu / config["trainer"]["acc_batches"]) * config["trainer"]["epochs"]
    )
